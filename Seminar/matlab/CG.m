...
CGx = (COMx*M + CG_wc(1,1)*w.Mc)/(M + w.Mc);
CGy = (COMy*M + CG_wc(2,1)*w.Mc)/(M + w.Mc); 


figure(1)
hold on
plot(X,Y,'b -o', 'LineWidth',2);
plot(Cx,Cy, 'k d', 'LineWidth',2);
plot(COMx, COMy, 'r x', 'LineWidth',2);
plot(CGx, CGy, 'g x', 'LineWidth',2);
